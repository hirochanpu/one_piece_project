...
img_path = os.path.join(base_dir, "図12.png") 
...
# =========================================================
# 💾 外部ファイルを使った「最下部ひっそり通算アクセス数」カウンター
# =========================================================
import os

# 1. カウントを保存するテキストファイルの場所（アプリと同じフォルダ）
counter_file = os.path.join(os.path.dirname(__file__), "counter.txt")

# 2. ファイルから現在の通算人数を読み込む関数
def read_global_counter():
...
# 3. ファイルに新しい通算人数を書き込む関数
def write_global_counter(count):
    try:
        with open(counter_file, "w", encoding="utf-8") as f:
            f.write(str(count))
    except Exception as e:
        print(f"[ERROR] カウンター書き込み失敗だぜ：{e}")

# 4. このセッションで初めてページを開いた時だけカウントを増やす
if "global_counted" not in st.session_state:
    current_total = read_global_counter()
    new_total = current_total + 1
    write_global_counter(new_total)
    
    st.session_state["global_counted"] = True
    st.session_state["display_total"] = new_total
elif "display_total" not in st.session_state:
...
st.markdown(f"""
    ...
    .hidden-footer {{
        ...
        z-index: 9999;                /* 他の要素の下に隠れないように最前面へ */
        pointer-events: none;         /* マウスの邪魔をしない設定 */
    }}
    </style>
    <div class="hidden-footer">
        🏴‍☠️ これまでの通算来航者数: {st.session_state['display_total']} 人
    </div>
